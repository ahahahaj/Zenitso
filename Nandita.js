...
require('./settings')
const { default: DeepakBotIncConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startDeepakBotInc() {
    ...
    // anticall auto block
    DeepakBotInc.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let blockxeon = await DeepakBotInc.sendContact(callerId, global.owner)
    DeepakBotInc.sendMessage(callerId, { text: `*Automatic blocking system!*\n*Don't call bot*!\n*Please contact the owner to open block !*`}, { quoted : blockxeon })
    await sleep(8000)
    await DeepakBotInc.updateBlockStatus(callerId, "block")
    }
    })
    ...
    DeepakBotInc.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                let targetname = await DeepakBotInc.getName(num)
...
DeepakBotInc.sendMessage(anu.id, buttonMessage)
...
{buttonId: `wkwkwk`, buttonText: {displayText: 'Bye Bye🤣'}, type: 1}
	...
	DeepakBotInc.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
    }
    ...
    DeepakBotInc.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startDeepakBotInc(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startDeepakBotInc(); }
            ...
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startDeepakBotInc(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startDeepakBotInc(); }
            else DeepakBotInc.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
        console.log('Connected...', update)
    })

    DeepakBotInc.ev.on('creds.update', saveState)
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        DeepakBotInc.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
        ...
        DeepakBotInc.sendMessage(jid, listMes, { quoted: quoted })
        }
     ...
     * @returns 
     */
        DeepakBotInc.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        DeepakBotInc.sendMessage(jid, templateMessage)
        }
     ...
     * @returns
     */
    DeepakBotInc.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: DeepakBotInc.waUploadToServer })
     ...
     * @returns
     */
    DeepakBotInc.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: DeepakBotInc.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
     ...
     * @returns
     */
    DeepakBotInc.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: DeepakBotInc.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
